...
import com.example.iquiz.dto.ai.EvaluateSubmissionResultDto;
...
public class AttemptUtil {
    ...
    public SubmissionResult evaluate(
        ...
        List<EvaluateSubmissionItemDto> aiEvaluationItems =
                new ArrayList<>();
        for (AttemptDetailDto dto : submissions) {
            Exercise exercise = Optional.ofNullable(
                    exerciseMap.get(dto.exerciseId())
            ).orElseThrow(() ->
            ...
            BigDecimal score;
            boolean requiresAI =
                    exercise.getType() == ExerciseType.SHORT_ANSWER
                            || exercise.getType() == ExerciseType.ESSAY;
            if (requiresAI) {
                aiEvaluationItems.add(
                        EvaluateSubmissionItemDto.builder()
                ...
                score = submissionUtil.calculateScore(
                        exercise,
                        userAnswerJson
                );
            }
        ...
        if (!aiEvaluationItems.isEmpty()) {
            List<EvaluateSubmissionResultDto> aiResults =
                    aIService.evaluateSubmissions(aiEvaluationItems);
            Map<UUID, EvaluateSubmissionResultDto> aiMap =
                    aiResults.stream()
                            .collect(Collectors.toMap(
                                    EvaluateSubmissionResultDto::getExerciseId,
                                    Function.identity()
                            ));
            for (AttemptDetail detail : details) {
                UUID exerciseId =
                        detail.getExercise().getId();
                EvaluateSubmissionResultDto aiResult =
                        aiMap.get(exerciseId);
                if (aiResult == null) {
                    continue;
                }

                detail.setScore(aiResult.getScore());
            }
        }
...
//        Feedback
        AttemptFeedbackDto attemptFeedback = null;
        for (AttemptDetail detail : details) {
            Exercise exercise = detail.getExercise();
            List<String> userAnswers =
                    submissionUtil.parseAnswers(
                            detail.getUserAnswerJson(),
                            exercise
                    );
            List<String> expectedAnswers =
                    submissionUtil.parseAnswers(
                            exercise.getCorrectAnswerJson(),
                            exercise
                    );
            feedbacks.add(
                    new ResultDto(
            ...
            attemptFeedback = aIService.generateAttemptFeedback(feedbacks);
        }
        ...
        return new SubmissionResult(details, feedbacks, categoryStats, finalScore, correctCount, wrongCount, attemptFeedback);
    }
