...
export const learningUnitService = createApi ({
    ...
    endpoints: (builder) => ({
        ...
        getAllLearningUnits: builder.query<LearningUnitDTO[], void>({
            ...
            providesTags: (result) =>
                result
                    ? [...result.map(({ id }) => ({ type: 'LearningUnit' as const, id })), { type: 'LearningUnit' as const, id: 'LIST' }]
...
export const {useGetAllLearningUnitsQuery, useGetLearningUnitByIdQuery,useLazyGetLearningUnitByIdQuery, useGetAllLearningUnitsByLevelQuery, useGetAllCompletedCoursesByUserIdQuery, useGetAllIncompletedCoursesByUserIdQuery} = learningUnitService;