...
import { useGetAllCompletedCoursesByUserIdQuery, useGetAllIncompletedCoursesByUserIdQuery } from "../../API/service/progress.service";
import { ProgressDTO } from "../../model/ProgressDTO";

type SortKey = keyof ProgressDTO;
...
const Profile = () => {
  ...
  const [sortConfig, setSortConfig] = useState<SortConfig>({ key: "lastActivityAt", direction: "asc" });
          ...
          <div className="flex space-x-2 mb-4">
            {(["week", "month", "year"] as TimeRange[]).map((range) => (
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm ${timeRange === range ? "bg-text-color text-white" : "bg-gray-100"
                  }`}
              >
        ...
        <div className="bg-white p-4 mt-2 rounded-t-xl shadow-md">
          <div className="flex flex-col mb-4">
            ...
            <div className="flex space-x-2 mt-2">
              <button onClick={() => requestSort("courseName")} className="px-2 py-1 rounded-md text-xs bg-gray-100">
                Sort by Name
              </button>
            </div>
          </div>
